...
export default function Globe3D() {
  ...
  useEffect(() => {
    ...
    import("three").then((THREE) => {
      ...
      const onDown = (e: PointerEvent) => { isDragging = true; prev = { x: e.clientX, y: e.clientY }; };
      const onMove = (e: PointerEvent) => { if (!isDragging) return; const dx = e.clientX - prev.x, dy = e.clientY - prev.y; globeGroup.rotation.y += dx * 0.005; globeGroup.rotation.x += dy * 0.005; prev = { x: e.clientX, y: e.clientY }; };
      const onUp = () => { isDragging = false; };
      const onWheel = (e: WheelEvent) => { e.preventDefault(); camera.position.z = Math.max(3.5, Math.min(10, camera.position.z + e.deltaY * 0.005)); };
